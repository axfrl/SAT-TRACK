            ...
            for dim_i, dim_j in zip(diagrams[i], diagrams[j]):
                if distance_type.lower() == "wasserstein":
                    if dim_i.size == 0 and dim_j.size == 0:
                        ...
                        distances.append(np.log(wasserstein(dim_i, dim_j, matching=False)))
                
                elif distance_type.lower() == "bottleneck":
                    if dim_i.size == 0 and dim_j.size == 0:
                        distances.append(0)
                    else:
                        distances.append(np.log(bottleneck(dim_i, dim_j)))
...
def compute_cross_distance_matrix(diagrams_D, diagrams_P, distance_type="wasserstein", epsilon=0.01):
    """

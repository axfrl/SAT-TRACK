...
class Model(nn.Module):
    ...
    def __init__(self, encoder, decoder,
        ...
        self.timing_log_file = os.path.join(self.output_dir, 'model_timing_log.txt')
        with open(self.timing_log_file, 'w') as f:
            f.write("Batch,Timestamp,Stage,Time (s)\n")
    ...
    def _track_time(self, batch_id, stage, start_time):
        """Track and log the execution time for a specific stage."""
        elapsed = time.time() - start_time
        self.timing[stage].append(elapsed)
        timestamp = time.strftime("%Y-%m-%d %H:%M:%S")
        with open(self.timing_log_file, 'a') as f:
            f.write(f"{batch_id},{timestamp},{stage},{elapsed:.4f}\n")
        return elapsed

    def _print_timing_stats(self):
        """Print timing statistics for all tracked stages."""
        print("\nModel Timing Statistics (in seconds):")
        for stage, times in self.timing.items():
            if times:
                ...
                print(f"  Batches: {len(times)}")
            else:
                print(f"{stage}: No data recorded")
    ...
    def get_scale_map(self, x_list):
        ...
        self._track_time(0, 'get_scale_map', start_time)
    ...
    def forward_encoder(self, samples, targets, use_gt = False):
        ...
        self._track_time(0, 'forward_encoder', start_time)
    ...
    def process_smpl(self, poses, shapes, cam_xys, cam_intrinsics, detach_j3ds = False):
        ...
        self._track_time(0, 'process_smpl', start_time)
    ...
    def forward(self, samples: NestedTensor, targets, sat_use_gt = False, detach_j3ds = False):
        ...
        self._track_time(0, 'decoder_forward', start_decoder)
        ...
        for lvl in range(hs.shape[0]):
            ...
            self._track_time(0, 'pose_head', start_pose)
        ...
        self._track_time(0, 'forward_total', start_time)
        return out

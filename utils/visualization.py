...
def vis_vertices_img_with_tracked_pose(frame, tracked_poses, cam_intrins, frame_size, colors, input_size=1288, point_radius=8):
    ...
    for id, joints_3d in tracked_poses.items():
        ...
        # Ajouter les indices à côté des points
        for idx, (x, y) in enumerate(coords):
            # Décalage pour éviter que le texte ne chevauche le point
            text_pos = (x + 10, y + 10)
            cv2.putText(
                frame,
                str(idx),
                ...
                0.5,  # Taille de la police
                color,  # Utiliser la couleur de l'ID
                1,  # Épaisseur du texte
                cv2.LINE_AA
            )
